...
void RS485_SendBuf1(uint8_t *_ucaBuf, uint16_t _usLen)
{
	comSendBuf(COM6, _ucaBuf, _usLen);
}
...
void RS485_ReciveNew1(uint8_t _byte)
{
...
static void UartVarInit(void)
{
	...
	g_tUart6.SendBefor = RS485_SendBefor1;		/* 发送数据前的回调函数 */
	g_tUart6.SendOver = RS485_SendOver1;		/* 发送完毕后的回调函数 */
	g_tUart6.ReciveNew =RS485_ReciveNew1;		/* 接收到新数据后的回调函数 */

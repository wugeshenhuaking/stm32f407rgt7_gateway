...
/* user include header file add begin */
#include "lcd_nt35510.h"
#include "bsp_lcd_port.h"
/* user include header file add end */
...
#define yPhys y
    ...
    GUI_USE_PARA(PixelIndex);
    {
        ...
        // TBD by customer...
        //
        BSP_LCD_PutPixelGUI(xPhys, yPhys, PixelIndex);
    }
...
#define yPhys y
    ...
    GUI_USE_PARA(y);
    {
        ...
        // TBD by customer...
        //
        PixelIndex = BSP_LCD_GetPixelGUI(xPhys, yPhys);
    }
#if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
#undef xPhys
#undef yPhys
...
#if emWin_Optimize && (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
...
#if emWin_Optimize && (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
        ColorIndex = LCD__GetColorIndex();
        BSP_LCD_FillRectGUI( x0,  y0,  x1,  y1,  ColorIndex);
#else
        for (; y0 <= y1; y0++)
        {

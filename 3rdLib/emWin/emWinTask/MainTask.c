...
*********************************************************************************************************
*/
GUI_RECT rClient    = {0,   0,  90, 38};     /* 用于显示示波器的logo */
GUI_RECT rRunMode   = {90,  0, 180, 38};     /* 用于显示运行状态，运行和暂停 */
GUI_RECT rTrigMode  = {180, 0, 270, 38};     /* 用于显示触发模式，自动触发和普通触发 */
GUI_RECT rTrigPos   = {270, 6, 790, 33};     /* 6KB数据查询：自动触发，普通触发 */
GUI_RECT rTrigValue = {790, 0, 960, 38};     /* 用于显示自动和正常的触发数组 */
GUI_RECT rButState  = {960, 0, 1140, 38};     /* 当前按键需要调节的状态 */


GUI_RECT rRefPos    = {0,  39, 39, 639};     /* 示波器最左侧波形参考的位置区域 */
...
static void _DrawWave(void)
{
    ...
    if(TimeBaseId < 10)
    {
        ...
        for(i = 0; i < DSOSCREEN_LENGTH; i++)
        {
            temp = rand() % 10;
            temp = temp + 140 - 40;
            g_DSO1->usWaveBuf[i] = 400 - temp + (140 - 240) * 2;
        }
        ...
        if(TriggerFlag == 0)
        {
            ...
            for(i = 0; i < DSOSCREEN_LENGTH; i++)
            {
                temp = rand() % 10;
                temp = temp + 340 - 40;
                g_DSO2->usWaveBuf[i] = 400 - temp + (340 - 240) * 2;
            }
...
//	}
    ...
    {
        GUI_DrawHLine(g_TrigVol->usPos, 40, 1026);
    }
...
//		}
//		printf("波形刷新时间 = %dms %d, %d, %d\r\n", t1, State.x, State.y, State.Pressed);
        printf("波形刷新时间 = %dms\r\n", t1);
...
static void _cbMotion(WM_MESSAGE * pMsg)
{
    ...
    case WM_MOTION:
        ...
        case WM_MOTION_MOVE:
            ...
            {
                if((pInfo->pState->Pressed == 1) && (pInfo->pState->x <= 540))
                {
                    pPara->iCH1Pos += pInfo->dy;
                    if(pPara->iCH1Pos >= 640)
                    {
                        pPara->iCH1Pos = 640;
                    }
                    if(pPara->iCH1Pos <= 40)
                ...
                else if((pInfo->pState->Pressed == 1) && (pInfo->pState->x > 540))
                {
                    pPara->iCH2Pos += pInfo->dy;
                    if(pPara->iCH2Pos >= 640)
                    {
                        pPara->iCH2Pos = 640;
                    }
...
*********************************************************************************************************
*/
extern __WEAK uint32_t osRtxMemoryInit (void *mem, uint32_t size);
extern __WEAK void *osRtxMemoryAlloc (void *mem, uint32_t size, uint32_t type);
extern __WEAK uint32_t osRtxMemoryFree (void *mem, void *block);
uint64_t AppMallocCCM[80 * 1024 / 8];
...
void MainTask(void)
{
    ...
    osRtxMemoryInit(AppMallocCCM, 1024 * 80);

    /* 申请示波器通道1动态内存 */
    g_DSO1 = (DSO_T *)osRtxMemoryAlloc(AppMallocCCM, sizeof(DSO_T), 0);

    /* 申请示波器通道2动态内存 */
    g_DSO2 = (DSO_T *)osRtxMemoryAlloc(AppMallocCCM, sizeof(DSO_T), 0);

    /* 申请游标测量结构体变量动态内存 */
    g_Cursors = (CURSORS_T *)osRtxMemoryAlloc(AppMallocCCM, sizeof(CURSORS_T), 0);

    /* 申请标志位结构体变量动态内存 */
    g_Flag = (FLAG_T *)osRtxMemoryAlloc(AppMallocCCM, sizeof(FLAG_T), 0);

    /* 申请触发结构体变量动态内存 */
    g_TrigVol = (TRIVOLTAGE_T *)osRtxMemoryAlloc(AppMallocCCM, sizeof(TRIVOLTAGE_T), 0);

    /* 申请FFT动态内存 */
    testInput_fft_2048 = (float32_t *)osRtxMemoryAlloc(AppMallocCCM, sizeof(float32_t) * 2048, 0);
    testOutput_fft_2048 = (float32_t *)osRtxMemoryAlloc(AppMallocCCM, sizeof(float32_t) * 2048, 0);

    /* 申请RMS动态内存 */
    g_RMSBUF = (float32_t *)osRtxMemoryAlloc(AppMallocCCM, sizeof(float32_t) * 600, 0);

    /* 申请FIR动态内存 */
    FirDataInput = (float32_t *)osRtxMemoryAlloc(AppMallocCCM, sizeof(float32_t) * FIR_LENGTH_SAMPLES, 0);
    FirDataOutput = (float32_t *)osRtxMemoryAlloc(AppMallocCCM, sizeof(float32_t) * FIR_LENGTH_SAMPLES, 0);
    firStateF32 = (float32_t *)osRtxMemoryAlloc(AppMallocCCM, sizeof(float32_t) * FIR_StateBufSize, 0);
    ...
    */
    ...
    hMemDSO1 = GUI_MEMDEV_CreateFixed(40,
                                      40,
                                      1040,
                                      640,
                                      GUI_MEMDEV_NOTRANS,
    ...
    DSO_DrawBakFrame(0, 0, 1000 - 1, 600 - 1);
    GUI_MEMDEV_Select(0);
